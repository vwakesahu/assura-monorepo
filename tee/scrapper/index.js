...
const { createPRAfterPoll } = require("./github-pr");
...
async function loadDataFromFile() {
  try {
    ...
    const { _metadata, ...addressData } = jsonData;

    // Reconstruct Map from JSON (excluding metadata)
    addressDatabase = new Map(Object.entries(addressData));
    console.log(
      `[${new Date().toISOString()}] Loaded ${
        addressDatabase.size
      } addresses from file.`
    );
  } catch (error) {
    if (error.code === "ENOENT") {
      console.log(
        `[${new Date().toISOString()}] No existing data file found. Starting fresh.`
      );
    } else {
      console.error(
        `[${new Date().toISOString()}] Error loading data:`,
        error.message
      );
    }
  }
...
async function saveDataToFile() {
  try {
    ...
    // Add updatedAt timestamp to track changes
    ...
    await fs.writeFile(DATA_FILE, JSON.stringify(dataWithTimestamp, null, 2));
    console.log(
    ...
    console.error(
      `[${new Date().toISOString()}] Error saving data:`,
      error.message
    );
  }
}
...
async function scrapeGitHub() {
  try {
    ...
    console.log(
      `[${new Date().toISOString()}] GitHub: Found ${totalCount} addresses, ${newCount} new.`
    );
    ...
    console.error(
      `[${new Date().toISOString()}] GitHub scraping error:`,
      error.message
    );
    return {
      success: false,
...
async function pollOFACXML() {
  try {
    ...
    const featureTypeValues =
      result?.Sanctions?.ReferenceValueSets?.FeatureTypeValues?.FeatureType;

    if (!featureTypeValues) {
      throw new Error("Could not find FeatureTypeValues in XML");
    }

    const featureTypes = Array.isArray(featureTypeValues)
      ? featureTypeValues
      : [featureTypeValues];
    ...
    featureTypes.forEach((ft) => {
      if (ft._ && ft._.includes("Digital Currency Address - ETH")) {
        ...
        console.log(
          `[${new Date().toISOString()}] Found ETH FeatureType ID: ${ethFeatureTypeId}`
        );
      }
    });
    ...
    const parties = Array.isArray(distinctParties)
      ? distinctParties
      : [distinctParties];
    ...
    parties.forEach((party) => {
      ...
      try {
        ...
        if (identity?.Alias) {
          const aliases = Array.isArray(identity.Alias)
            ? identity.Alias
            : [identity.Alias];
      ...
      const profileFeatures =
        party.Profile?.Feature || party.Profile?.Features?.Feature;

      if (profileFeatures) {
        const features = Array.isArray(profileFeatures)
          ? profileFeatures
          : [profileFeatures];
        ...
        features.forEach((feature) => {
          ...
          if (String(featureTypeID) === String(ethFeatureTypeId)) {
            ...
            if (featureVersion) {
              const versions = Array.isArray(featureVersion)
                ? featureVersion
                : [featureVersion];

              versions.forEach((version) => {
                const versionDetails = version.VersionDetail
                  ? Array.isArray(version.VersionDetail)
    ...
    console.log(
      `[${new Date().toISOString()}] OFAC XML: Found ${totalCount} addresses, ${newCount} new.`
    );
    ...
    console.error(
      `[${new Date().toISOString()}] OFAC XML polling error:`,
      error.message
    );
    return {
      success: false,
...
async function pollAndSave() {
  ...
  const results = await Promise.all([scrapeGitHub(), pollOFACXML()]);
  ...
  console.log(
    `[${new Date().toISOString()}] Poll cycle complete. Total addresses: ${
      addressDatabase.size
    }, New: ${totalNew}`
  );

  // Always attempt to push changes directly to upstream after each poll
  console.log(
    `[${new Date().toISOString()}] Attempting to push changes to upstream...`
  );
  try {
    const pushResult = await createPRAfterPoll(results);
    if (pushResult.success) {
      console.log(
        `[${new Date().toISOString()}] Successfully pushed changes to upstream repository.`
      );
    } else {
      if (pushResult.reason === "no_changes") {
        console.log(
          `[${new Date().toISOString()}] No changes to commit. Skipping push.`
        );
      } else {
        console.warn(
          `[${new Date().toISOString()}] Failed to push: ${
            pushResult.reason || pushResult.error
          }`
        );
      }
    }
    ...
    console.error(
      `[${new Date().toISOString()}] Error pushing changes:`,
      error.message
    );
  }
...
async function startPolling() {
  ...
  console.log(
    `Polling started. Will poll every ${POLL_INTERVAL / 1000} seconds (${
      POLL_INTERVAL / 60000
    } minutes).`
  );
}
...
app.get("/data", (req, res) => {
  ...
  res.json({
    ...
    lastUpdated:
      addresses.length > 0
        ? addresses.reduce(
            (latest, addr) => (addr.addedAt > latest ? addr.addedAt : latest),
            addresses[0].addedAt
          )
        : null,
    addresses: addresses,
  ...
  res.json({
    ...
    endpoints: {
      "GET /check-address/:address":
        "Check if a specific address is sanctioned",
      "GET /addresses":
        "Get all addresses (supports ?entity=, ?source=, ?limit=)",
    ...
    polling: {
      ...
      interval: `${POLL_INTERVAL / 1000} seconds (${
        POLL_INTERVAL / 60000
      } minutes)`,
