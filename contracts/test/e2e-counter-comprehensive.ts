...
describe("Comprehensive Counter E2E Tests on Base Sepolia", async function () {
  ...
  it("Should verify that verifying data was set correctly", async function () {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        incSelector = await (counterContract.read as any).getOnlyUserWithScore20Selector();
        incBySelector = await (counterContract.read as any).getOnlyUserWithScore40Selector();
  ...
  it("Should successfully call inc() with valid EIP-191 compliance data", async function () {
    ...
    const key = await (counterContract.read as any).getOnlyUserWithScore20Selector();
  ...
  it("Should successfully call incBy() with valid compliance data", async function () {
    ...
    const key = await (counterContract.read as any).getOnlyUserWithScore40Selector();
  ...
  it("Should fail with wrong key", async function () {
    ...
    const wrongKey = await (counterContract.read as any).getOnlyUserWithScore40Selector(); // Wrong selector for inc()
    const correctKey = await (counterContract.read as any).getOnlyUserWithScore20Selector();
  ...
  it("Should deploy and test Vault with Delayed Deposit Hooks", async function (this: { skip: () => void }) {
    ...
    const assetContract = await viem.getContractAt("MockERC20", assetAddress) as any;
    ...
    const vaultContract = await viem.getContractAt("AssuraProtectedVaultWithHooks", vaultAddress) as any;
    ...
    // Attempt deposit
    const key = await (vaultContract.read as any).getOnlyUserWithScore20Selector();
    const attestedData = {
      score: userScore,
      timeAtWhichAttested: BigInt(attestation.attestedData.timeAtWhichAttested),
      chainId: BigInt(attestation.attestedData.chainId),
